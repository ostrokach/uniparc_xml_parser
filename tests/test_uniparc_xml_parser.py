...
from biorustlings import uniparc_xml_parser
from kmtools import sequence_tools, system_tools
...
class TestUniParcXMLParser:
    ...
    @pytest.mark.parametrize(
        "match,output", [('''type="EMBL" id="AAF63732" version_i="1" active="Y" version="1" '''
                         ...
                         ('''type="protein_name" value="aminotransferase family protein\\"''', {
                             'type': 'protein_name',
                             'value': 'aminotransferase family protein'
                         })])
    ...
    def test__append_to_file(self):
        self.parser = sequence_tools.UniParcXMLParser(self.file_path, self.output_dir, 'csv')
        data = [
            {
                'uniparc_id': 1
            },
    ...
    def test_run(self, writer):
        with system_tools.open_compressed(self.file_path, 'rt') as ifh:
            self.parser = sequence_tools.UniParcXMLParser(ifh, self.output_dir, 'pandas')
            t0 = time.time()
            self.parser.parse()
            t1 = time.time()
    ...
    @pytest.mark.skipif(shutil.which('pypy') is None, reason="`pypy` must be installed.")
    ...
    def test_run_pypy(self, optimize):
        ...
        system_command = ("zcat {} | pypy {} '{}' --output_dir '{}'".format(
            self.file_path, optimize, uniparc_xml_parser.__file__, self.output_dir))
    ...
    def _assert_dataframes_match(self):
        ...
        for filename in filenames:
            df_ref = pd.read_csv(
                ...
                sep='\t')
            df_test = pd.read_csv(op.join(self.output_dir, filename), sep='\t')
